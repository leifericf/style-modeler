...
You are packaging an existing StyleModeler profile under `artefacts/` into a single distributable unit that downstream consumers (humans and AI agents) can use with minimal friction.
...
1) Verify `artefacts/` exists and contains at least one per-language folder `artefacts/<lang>/`.
2) Identify available languages by listing `artefacts/*/generation_blocks.md`.
...
2) Language(s): default to all languages found under `artefacts/`
3) Include optional sensitive-ish artifacts? (recommended defaults)
...
- Safety:
  ...
  - do not output artifact contents
...
4) Embedded artifacts for each included file, using these markers:
